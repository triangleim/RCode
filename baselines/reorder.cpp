...
POrder porder;
std::string extract_only_filename(const std::string full_filename)
{
    int pos0 = full_filename.find_last_of('/');
    return "../gro_data"+full_filename.substr(pos0, full_filename.length());
}
int main(int argc, char* argv[])
{
    ...
    save_newid(extract_only_filename(reordered_newid_file_path), porder.org2newid);
    gettimeofday(&time_end, NULL);
